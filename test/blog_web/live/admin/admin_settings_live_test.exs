defmodule BlogWeb.Admin.AdminSettingsLiveTest do
  ...
  describe "Settings page" do
    ...
    test "redirects if admin is not logged in", %{conn: conn} do
      assert {:error, redirect} = live(conn, ~p"/admin/admins/settings")

      assert {:redirect, %{to: path, flash: flash}} = redirect
      assert path == ~p"/admin/admins/log_in"
  ...
  describe "update email form" do
    ...
    test "updates the admin email", %{conn: conn, password: password, admin: admin} do
      ...
      {:ok, lv, _html} = live(conn, ~p"/admin/admins/settings")
  ...
  describe "update password form" do
    ...
    test "updates the admin password", %{conn: conn, admin: admin, password: password} do
      ...
      assert redirected_to(new_password_conn) == ~p"/admin/admins/settings"
  ...
  describe "confirm email" do
    ...
    test "updates the admin email once", %{conn: conn, admin: admin, token: token, email: email} do
      {:error, redirect} = live(conn, ~p"/admin/admins/settings/confirm_email/#{token}")

      assert {:live_redirect, %{to: path, flash: flash}} = redirect
      assert path == ~p"/admin/admins/settings"
    ...
    test "does not update email with invalid token", %{conn: conn, admin: admin} do
      {:error, redirect} = live(conn, ~p"/admin/admins/settings/confirm_email/oops")
      assert {:live_redirect, %{to: path, flash: flash}} = redirect
      assert path == ~p"/admin/admins/settings"
      assert %{"error" => message} = flash
      assert message == "Email change link is invalid or it has expired."
